...
def generate_ind228(row):
    if row['IND1'] == 1:
        return row['WNACWindDirection']


def generate_ind230(row):
    return gap(row, 'IND228', 1000)


def generate_ind263(row):
    return interval_count(row, 'IND230', 6000, 2)


def generate_ind276(row):
    return gap(row, 'IND6', 1000)


def generate_ind277(row):
    return interval_count(row, 'IND276', 6000, 2)


def generate_ind672(row):
    return gap(row, 'IND13', 1000)


def generate_ind673(row):
    return interval_count(row, 'IND672', 6000, 2)


def generate_ind2822(row):
    return gap(row, 'IND8', 1000)


def generate_ind2823(row):
    return interval_count(row, 'IND2822', 6000, 2)


def create_conditions(output_dir):
    global input_data
    input_data['IND1'] = input_data.apply(generate_ind1, axis = 1)
    # input_data['IND6'] = input_data.apply(generate_ind6, axis = 1)
    # input_data['IND8'] = input_data.apply(generate_ind8, axis = 1)
    # input_data['IND13'] = input_data.apply(generate_ind13, axis = 1)
    # input_data['IND228'] = input_data.apply(generate_ind228, axis = 1)
    # input_data['IND230'] = input_data.apply(generate_ind230, axis = 1)
    # input_data['IND263'] = input_data.apply(generate_ind263, axis = 1)
    # input_data['IND276'] = input_data.apply(generate_ind276, axis = 1)
    # input_data['IND277'] = input_data.apply(generate_ind277, axis = 1)
    # input_data['IND672'] = input_data.apply(generate_ind672, axis = 1)
    # input_data['IND673'] = input_data.apply(generate_ind673, axis = 1)
    # input_data['IND2822'] = input_data.apply(generate_ind2822, axis = 1)
    # input_data['IND2823'] = input_data.apply(generate_ind2823, axis = 1)
